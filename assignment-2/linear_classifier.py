...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margins = scores - correct_scores.view(N, 1) + 1
  margins[margins <= 0] = 0
  margins[range(N), y] = 0
  
  loss = torch.mean(margins.sum(axis=1))
  loss += reg * torch.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-4, 1e-3, 1e-2, 3e-4, 3e-3, 3e-2]
  regularization_strengths = [3e-3, 3e-2, 1e-3, 1e-2]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(N):
    ...
    scores_prob = torch.exp(scores)/torch.exp(scores).sum()
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores -= max_values.view(N, 1)

  exp = torch.exp(scores)
  scores_prob = exp/exp.sum(dim=1).view(N, 1)

  loss = torch.sum(-torch.log(scores_prob[range(N), y]))
