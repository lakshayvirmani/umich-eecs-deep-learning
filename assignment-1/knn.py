...
def compute_distances_two_loops(x_train, x_test):
  ...
  for train_idx, train_img in enumerate(x_train):
    for test_idx, test_img in enumerate(x_test):
      dists[train_idx][test_idx] = torch.sqrt(torch.sum(torch.square(train_img - test_img)))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for train_idx, train_img in enumerate(x_train):
    train_img_flat = train_img.view(1, -1)
    test_images_flat = x_test.view(num_test, -1)
    computed_dists = torch.sqrt(torch.sum(torch.square(train_img_flat - test_images_flat), axis=1))
    dists[train_idx] = computed_dists
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_sqr = x_train_flat.square().sum(axis = 1)
  x_test_sqr = x_test_flat.square().sum(axis = 1)
  x_train_test_dot = x_train_flat.mm(x_test_flat.T)

  dists = torch.sqrt(x_train_sqr.view(num_train, -1) + x_test_sqr.view(-1, num_test) - 2 * x_train_test_dot)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  values, indices = torch.topk(dists.T, k, largest=False)
  for i in range(num_test):
    predictions = y_train[indices[i]]
    y_pred[i] = torch.argmax(torch.bincount(predictions))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
    ...
    for i in range(num_folds):
      ...
      for j in range(num_folds):
        if i == j:
          ...
          if x_train_fold is not None:
            x_train_fold = torch.cat((x_train_fold, x_train_folds[j]))
            y_train_fold = torch.cat((y_train_fold, y_train_folds[j]))
          else:
