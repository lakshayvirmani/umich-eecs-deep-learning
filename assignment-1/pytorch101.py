...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  new_start = start + start % 10
  if new_start <= stop:
    x = torch.arange(new_start, stop, 10).to(torch.float64)
  else:
...
def slice_assignment_practice(x):
  ...
  x[:2, 2:6] = 2
  x[2:4, :2] = x[2:4, 2:4] = torch.tensor([[3, 4], [3, 4]])
  x[2:4, 4:6] = 5
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
